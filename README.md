...
一个功能强大的 macOS 剪贴板历史管理工具，支持快捷键唤起、多种数据类型。
...
5. **选择粘贴**: 
   - 使用 `←` `→` 方向键左右选择项目（横向滚动）
   - 按 `回车` 粘贴选中的项目，或直接 **单击** 项目进行粘贴
   - 按 `ESC` 关闭窗口

### 实用场景

- **💻 编程开发**: 复制函数名、代码片段、命令，使用 `⌘⌥V` 快速找回之前复制的代码。
- **📝 写作文档**: 收集多个资料、引用、链接，使用搜索功能快速定位并粘贴。
- **🎨 设计工作**: 复制多个图片、文件路径，随时切换和使用，无需反复查找文件。
...
|--------|------|
| `⌘⌥V` | 显示/隐藏剪贴板历史窗口（屏幕底部） |
| `⌥V` | （可选开关）模拟 `⌘Space` 后延迟模拟 `⌘4` |
| `←` `→` | 在历史列表中左右选择（横向滚动） |
| `回车` | 粘贴选中的项目 |
| `ESC` | 关闭历史窗口 |
| `Delete(⌫)` / `Del(⌦)` | 删除选中的历史项（MacBook 上 `fn+Delete` 等同于 `Del(⌦)`） |
